...
│   │
│   ├── external            <- Data from third party sources (which is different from raw in that it is not the original data dump)
│   │   │
│   │   └──
│   │
...
│   │   └── index.md            <- 
│   │
│   ├── project-architecture-diagram <- Directory for the diagrams related to project-description.md
│   │   │       
...
Old project is referred to as `mlops_old`.
...
- `docs/project-architecture.png` to `docs/project-architecture-diagram/project-architecture.png`
- `docs/diagrams.excalidraw` to `docs/project-architecture-diagram/diagrams.excalidraw`
...
- `notebooks/artifacts/X_test.csv` to `tests/test_data/X_test.csv` (contains test data for testing purposes)
- `notebooks/artifacts/y_test.csv` to `tests/test_data/y_test.csv` (contains test data for testing purposes)
...
- `README.md` to `project-description.md` (its the project description so it had to be renamed)

Created in `ITU_DSiP-100_LeGIT_project`:
- `mlops_old/dagger` (contains the dagger workflow files)
- `tests/test_data` (contains data for testing purposes)
...
- `action.yml` (it's empty, so for now we ignore it. If it's for github actions we will need to make a new one anyways)
